...
// assets
import { ReactComponent as Logo } from "./assets/blue-stream-logo.svg";
...
const App = () => {
        ...
        if (event.type === "flagged_message") {
          ...
          if (flagged.indexOf(message) === -1) {
            ...
            if (channelsClone.length) {
              const index = channelsClone.findIndex(
                (channel) => channel.cid === message.cid
              );
  ...
  const setup = async () => {
    ...
    channelsResponse.forEach(
      (channel) => (channel.flagged_count = flaggedMap[channel.cid] | 0)
    );
  ...
  return (
    <>
      <section className="logo-container">
        <Logo />
        <h1 className="header-text">Stream Moderator Dashboard</h1>
      </section>
      <main>
        {toast.show && <Toast text={toast.text} success={toast.success} />}
        <ChannelList
          channels={channels}
